...
def main_menu_keyboard() -> InlineKeyboardMarkup:
    ...
    for key, product in PRODUCTS.items():
        if first:
            rows.append([InlineKeyboardButton(
                ...
                icon_custom_emoji_id="5783063963872799297"
            )])
            first = False
        else:
            rows.append([InlineKeyboardButton(
    ...
    rows.append([InlineKeyboardButton("💱 نرخ ارز", callback_data="exchange_rate", icon_custom_emoji_id="6006098282659450411")])
    rows.append([InlineKeyboardButton("🆘 پشتیبانی", url=f"https://t.me/{SUPPORT_USERNAME}", icon_custom_emoji_id="6005949341783559382")])
